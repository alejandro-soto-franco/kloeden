...
void BM_gbm_milstein_scalar(benchmark::State& state) {
...
BENCHMARK(BM_gbm_milstein_scalar)->Unit(benchmark::kMillisecond)->MinWarmUpTime(0.1)->MinTime(1.0);
...
void emit_csv() {
    ...
    auto out = results_dir() / (std::string(KLOEDEN_IMPL_NAME) + "_milstein_gbm_scalar.csv");
    std::ofstream f(out, std::ios::trunc);
    f << "impl,width,scheme,process,payoff,n_paths,n_steps,ns_per_path_step,paths_per_s,mc_mean,mc_stderr\n";
    f << KLOEDEN_IMPL_NAME << ",scalar,milstein,gbm,none,"
      << m.n_paths << "," << m.n_steps << ","
