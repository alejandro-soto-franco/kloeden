...
//
// Weak-mean test: E[X_T] = x0 * exp(mu * T) for GBM, any scheme.
// Assert the sample mean is within 4 * stderr of the analytic value.
...
std::filesystem::path fixtures_dir() {
    ...
    // Default: ../fixtures relative to the build directory.
    return std::filesystem::current_path() / ".." / "fixtures";
}

} // namespace

TEST(GbmEulerScalar, TerminalMeanMatchesAnalytic) {
    ...
    // 4-sigma tolerance is the standard MC pass band. See elworthy BENCHMARK.md.
